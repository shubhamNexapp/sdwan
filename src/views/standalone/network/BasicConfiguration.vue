...
<template>
  ...
  <div v-if="service" class="mt-4 flex flex-col gap-y-3">
    <NeCombobox v-model="protocol" :options="[
      { label: 'ping', id: 'ping' },
      { label: 'http', id: 'http' },
      { label: 'dns', id: 'dns' },
    ]" :label="t('Protocol')" class="grow" />

    <p v-if="errorBag.protocol" class="text-sm" style="color: rgba(190, 18, 60, 0.9)">
      {{ errorBag.protocol }}
    </p>

    <NeCombobox multiple v-model="participant" :options="participantOptions" value-key="id" label-key="label"
      :label="$t('Participants')" class="grow" :noResultsLabel="$t('ne_combobox.no_results')"
    ...
    <p class="mt-2 flex flex-wrap gap-2 text-gray-700">
      <span v-for="(item, index) in baseOptions" :key="index"
    ...
    <NeTextInput v-model="server" :label="t('Server')" :placeholder="t('Enter server')"
      :invalidMessage="errorBag.server" />

    <NeToggle v-model="slaTarget" :topLabel="t('SLA Target')" :label="slaTarget ? t('Enabled') : t('Disabled')"
    ...
    <div class="grid grid-cols-12 gap-x-6 gap-y-4">
      ...
      <div class="col-span-12 flex items-center">
        ...
        <NeToggle class="mr-4" v-model="showLatency" :label="showLatency ? t('Enabled') : t('Disabled')" />
        <NeTextInput class="w-1/3" v-model="latencyThreshold" :placeholder="t('Enter latency in ms')"
          :invalidMessage="errorBag.latencyThreshold" :disabled="!showLatency" @input="onlyNumbers" />
      </div>
      ...
      <div class="col-span-12 flex items-center">
        ...
        <NeToggle class="mr-4" v-model="showJitter" :label="showJitter ? t('Enabled') : t('Disabled')" />
        <NeTextInput class="w-1/3" v-model="jitterThreshold" :placeholder="t('Enter jitter in ms')"
          :invalidMessage="errorBag.jitterThreshold" :disabled="!showJitter" @input="onlyNumbers" />
      </div>

      <!-- Packet Loss -->
      <div class="col-span-12 flex items-center">
        <label class="w-1/4 font-medium">{{ t("Packet Loss (%)") }}</label>
        <NeToggle class="mr-4" v-model="showPacketLoss" :label="showPacketLoss ? t('Enabled') : t('Disabled')" />
        <NeTextInput class="w-1/3" v-model="packetLoss" :placeholder="t('Enter packet loss')"
          :invalidMessage="errorBag.packetLoss" :disabled="!showPacketLoss" @input="onlyNumbers" />
      </div>
    </div>
    ...
    <NeTextInput v-model="checkInterval" :label="t('Check Interval (s)')" :placeholder="t('Enter interval in seconds')"
      :invalidMessage="errorBag.checkInterval" @input="onlyNumbers" />

    <NeTextInput v-model="failuresBeforeInactive" :label="t('Failures Before Inactive')"
      :placeholder="t('Enter failure count')" :invalidMessage="errorBag.failuresBeforeInactive"
      :invalid="!!errorBag.failuresBeforeInactive" @input="onlyNumbers" />

    <NeTextInput v-model="restoreLinkAfter" :label="t('Restore Link After (s)')"
      :placeholder="t('Enter restore time in seconds')" :invalidMessage="errorBag.restoreLinkAfter"
      @input="onlyNumbers" />
