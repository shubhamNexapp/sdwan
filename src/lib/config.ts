...
export const getSDControllerApiEndpoint = () => {
  if (import.meta.env.DEV) {
    // Nexapp SD-controller development environment

    const apiScheme = import.meta.env.VITE_API_SCHEME
    const controllerApiHost = import.meta.env.VITE_CONTROLLER_API_HOST
    return `${apiScheme}://${controllerApiHost}/api-new`
  } else {
    // Nexapp SD-controller production environment
