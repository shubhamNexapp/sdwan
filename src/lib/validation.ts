...
export const validateHostname = (hostname: any): validationOutput => {
  ...
  if (!hostname || hostname.trim() === '') {
    return { valid: false, errMessage: 'Hostname is required' };
  }

  if (hostname.length > 64) {
    return { valid: false, errMessage: 'Hostname must not exceed 64 characters.' };
  }

  const invalidCharRegex = /^[a-zA-Z0-9-]+$/;
  if (!invalidCharRegex.test(hostname)) {
    return { valid: false, errMessage: 'Hostname can only contain letters (A–Z, a–z), digits (0–9), and hyphens (-).' };
  }

  if (!/^[a-zA-Z0-9]/.test(hostname)) {
    return { valid: false, errMessage: 'Hostname cannot start with a hyphen.' };
  }

  if (!/[a-zA-Z0-9]$/.test(hostname)) {
    return { valid: false, errMessage: 'Hostname cannot end with a hyphen.' };
  }
...
export const validateMacAddress = (macAddress: String): validationOutput => {
  const isValid =
    macAddress.match(/^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/) != null;
...
export const validateIpAddressRange = (value: String): validationOutput => {
  if (
    !validateIp4AddressRange(value).valid &&
    !validateIp6AddressRange(value).valid
  ) {
...
export const validateIpv4SubnetMask = (
  subnetMask: String
): validationOutput => {
  const re =
...
export const validateHexadecimalString = (
  hexValue: String
): validationOutput => {
...
export const validateLeaseTime = (value: string): validationOutput => {
  ...
  if (value != "infinity") {
    ...
    if (
      (unit === "s" &&
        Number.parseInt(value.slice(0, value.length - 1)) < 120) ||
...
export const validatePassword = (value: String): validationOutput => {
  ...
  if (!value.match(/[^a-zA-Z0-9]/)) {
    return {
      valid: false,
      errMessage: "error.password_special_character_required",
    };
  }
  return { valid: true };
};

export const validateStringEqual = (
  value: String,
  otherValue: String
): validationOutput => {
  if (value != otherValue) {
...
export const validateFile = (
  ...
  if (format) {
    ...
    const extension = filenameSplit
      .slice(
        filenameSplit.length - format.split(".").length,
...
export const validateDNSForwardingServer = (
  value: string
): validationOutput => {
  const validResult = { valid: true };
  const invalidResult = {
    valid: false,
    errMessage: "error.invalid_dns_forwarding",
  };
