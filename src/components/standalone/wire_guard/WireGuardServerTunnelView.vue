...
const validate = () => {
  ...
  if (!listenPort.value) {
    ...
    errorBag.value.listenPort = 'Listen Port must be a number between 1 and 65536.';
  }
...
<template>
  <div class="space-y-6 mb-4">
    ...
    <template v-if="service">
      <NeTextInput v-model.trim="address" :label="t('Address')" :placeholder="t('Enter Address')"
      ...
      <NeTextInput v-model.trim="listenPort" :label="t('Listen Port')" :placeholder="t('Enter Listen Port')"
      ...
      <NeTextInput v-model.trim="mtu" :label="t('MTU')" :placeholder="t('Enter MTU')" :invalidMessage="errorBag.mtu">
        <template #tooltip>
          <NeTooltip>
            <template #content>
              {{ t('Enter MTU') }}
            ...
            <NeTableRow v-for="(item, index) in apiResponse[0].client" :key="index">
              ...
              <NeTableCell>
                <div :class="['flex', 'flex-row', 'items-center', ...getCellClasses(item)]">
                  <font-awesome-icon :icon="['fas', item.peer_service === 'enabled' ? 'circle-check' : 'circle-xmark']"
