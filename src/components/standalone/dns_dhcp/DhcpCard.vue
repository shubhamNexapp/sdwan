...
<template>
  <div
    :class="borderColor"
    class="px-4 py-5 space-y-4 text-sm text-gray-700 bg-white border-l-4 dark:bg-gray-950 dark:text-gray-200 sm:rounded-lg sm:px-6 sm:shadow"
  >
    <div class="flex justify-between">
      ...
      <div>
        <NeButton kind="tertiary" size="sm" @click="$emit('edit', dhcpInterface)">
          <template #prefix>
            <FontAwesomeIcon :icon="faPenToSquare" aria-hidden="true" class="w-4 h-4" />
          </template>
          {{ t('common.edit') }}
        </NeButton>
      </div>
